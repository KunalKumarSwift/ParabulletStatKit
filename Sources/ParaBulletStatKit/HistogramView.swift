...
public struct HistogramView: View {
    ...
    @MainActor
    private func calculateFrequencies() async {
#if os(macOS)
        // Use background thread on macOS
        DispatchQueue.global(qos: .userInitiated).async {
            let calculatedFrequencies = self.performFrequencyCalculation()
            DispatchQueue.main.async {
                self.frequencies = calculatedFrequencies
            }
        }
#else
        // Use async/await on iOS
        let calculatedFrequencies = await performFrequencyCalculationAsync()
        self.frequencies = calculatedFrequencies
#endif
    }

    private func performFrequencyCalculation() -> [(bin: Double, count: Int)] {
        var calculatedFrequencies: [(bin: Double, count: Int)] = []
        for value in stride(from: binStart, through: binEnd, by: binStep) {
            ...
            calculatedFrequencies.append((bin: value, count: count))
        }
        return calculatedFrequencies
    }

    private func performFrequencyCalculationAsync() async -> [(bin: Double, count: Int)] {
        return await withCheckedContinuation { continuation in
            let result = performFrequencyCalculation()
